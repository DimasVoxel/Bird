...
function scanAll(cacheToRegistry)
    ...
    local elapsed = Tock(MAIN_CLOCK)
    local elapsed_snds = elapsed / 1000000000
    DebugPrint("Mesh Scan: "..elapsed_snds)
   -- AutoInspectWatch(octGroup," asd",1 ," ")
    perTickLimit = 5000
    calculateGrid() 
    local elapsed = Tock(MAIN_CLOCK)
    local elapsed_grid = elapsed / 1000000000 - elapsed_snds
    DebugPrint("Grid: "..elapsed_grid)

    perTickLimit = 5000
    determinNeighbours()
    local elapsed = Tock(MAIN_CLOCK)
    local elapsed_neigh = elapsed / 1000000000 - elapsed_grid - elapsed_snds
    DebugPrint("Neighbour: "..elapsed_neigh)
...
function recursiveSearch(cache,t,depth)
    ...
    local function Round(v, increment)
    ...
    local function VecRound(vec, r)
        return Vec(Round(vec[1], r), Round(vec[2], r), Round(vec[3], r))
    end
    ...
    for i=1, #checkShape[depth].shapes do                           -- After that we do a is a shape touching check. This is done because 
        ...
        if #shapes == 0 then
            calculateCost(VecRound(min),VecRound(max),cache,cost,depth)  -- If no shape was found we give it a cost of the current depth
            ...
            if IsPointInWater(min) then 
                ...
                if none == true then 
                    actuallyDidSomething[#actuallyDidSomething+1] = {}
                    actuallyDidSomething[#actuallyDidSomething].min = min 
                    actuallyDidSomething[#actuallyDidSomething].max = max
                end
            end
            if none == true then 
                calculateCost(VecRound(min),VecRound(max),cache,cost,depth)
           -- elseif water then 
           --     calculateCost(AutoVecRound(min),AutoVecRound(max),cache,-2,depth)
            elseif depth == #checkShape then
                calculateCost(VecRound(min),VecRound(max),cache,-1,depth)   -- -1 means this is not navigatable
            end
        end
    end
    return cache
