...
---@param max? number
---@return table
function shared_buffer( name, max )
	max = max or 64
	return {
...
if coreloaded then
	...
	---@return table
	function shared_channel( name, max, local_realm )
		max = max or 64
		local channel = {
			_buffer = shared_buffer( name, max ),
			_offset = 0,
			_hooks = {},
			_ready_count = 0,
			_ready = {},
				...
				if last_pos > channel._offset then
					for i = math.max( channel._offset, last_pos - max ), last_pos - 1 do
						...
						if realms == ",," or realms:find( local_realm, 1, true ) then
							receive( unserialize( message:sub( start + 1 ) ) )
							if channel._ready_count <= 0 then
								channel._offset = i + 1
								return
							end
	...
	---@return table
	function async_channel( channel )
		local listener = {
			_channel = channel,
			_waiter = nil,
			read = function( self )
...
do
	...
	function register_unserializer( type, callback )
	...
	---@param base? table
	---@return table
	function shared_table( name, base )
	...
	function structured_table( name, base )
